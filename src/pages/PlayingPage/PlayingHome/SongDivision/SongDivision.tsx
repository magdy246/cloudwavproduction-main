...
export default function SongDivision() {
  ...
  const {
    ...
    queryKey: [`division-songs-${genre}`],
    queryFn: () => axiosServices.get("/Songs"),
    select: (data) => {
      return data?.data.filter((el:any) => el.division === genre);
    },
  });
  ...
  return (
    <div className="pb-24">
      ...
      {isFetching ? (
        ...
        <div className="flex flex-col items-center justify-center h-64">
          <p className="text-gray-500">No {genre} songs available</p>
        </div>
      ) : (
